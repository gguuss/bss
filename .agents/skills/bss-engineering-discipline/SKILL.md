...
- Specific regression rules:
  ...
  - **Bullseye Interaction & Event Shielding**: Bullseye capture must utilize a transparent event shield (`BullseyeTrackingOverlayWindow`) so mouse clicks, drags, and moves are captured reliably without clicking through into underlying applications or failing when accessibility privileges are absent. Initial menu item clicks must not abort the session on mouse-up, supporting both click-to-capture and drag-to-capture modes.
  - **Permissions Management & TCC Handling**: Permission checks must verify both CoreGraphics preflight and modern ScreenCaptureKit access. Passive events (such as window focus or activation) MUST only execute non-intrusive preflight checks (`CGPreflightScreenCaptureAccess`) to prevent infinite OS permission prompt dialog loops. Bundle identifiers must remain strictly aligned (`And-Friends.Better-Screen-Shot`) across project blueprints, entitlements, and build scripts to preserve system TCC grants and developer team signatures.
